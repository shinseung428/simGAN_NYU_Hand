...
class DepthMapHandle(object):
    ...
    def saveDepthMap8bit(self, dpt, com):

        # print dpt.shape
        mask = np.bitwise_and(dpt>900 , dpt != 0)
        dpt[mask] = 0

        dpt32 = np.zeros((self.img_size,self.img_size), np.float32)

        min_ = com[2] - self.cube_size
        max_ = com[2] + self.cube_size
        dpt32 = ((dpt - min_) / (max_ - min_)).reshape(self.img_size, self.img_size)



        cv2.imshow("test",dpt32)
        cv2.waitKey(1)

        # print dpt32*(self.cube_size / 2)
...
if __name__ == '__main__':
    ...
    #data_path = '/home/sshin/simGAN/data/hand/nyu_hand/dataset/train'
    numTotalJoint = 36
    ...
    for line in range(joints3D.shape[0]):
        ...
        dpt  = dmh.saveDepthMap8bit(dpt, com)

        saveName = "./data/hand/nyu_hand/png_cropped_dataset/tmp/crop_{0:05d}.png".format(line)
        print dpt
