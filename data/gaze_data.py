...
def process_json_list(json_list, img):
  #eval() function evaluates the passed string as a python expression and returns the result
  ldmks = [eval(s) for s in json_list]
  return np.array([(x, img.shape[0]-y, z) for (x,y,z) in ldmks])

#function that downloads a real dataset if it does not exists in the folder
...
#preprocessing images before training
